...
pthread_rwlock_t i_table_locker = PTHREAD_RWLOCK_INITIALIZER;
pthread_rwlock_t *i_lockers;
static allocation_state_t *freeinode_ts;

// Data blocks
static char *fs_data; // # blocks * block size
static pthread_rwlock_t *b_lockers;
...
pthread_mutex_t *of_lockers;
pthread_rwlock_t of_table_locker = PTHREAD_RWLOCK_INITIALIZER;
...
bool valid_file_handle(int file_handle) {
...
int state_init(tfs_params params) {
    ...
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        pthread_rwlock_init(&i_lockers[i], NULL);
        freeinode_ts[i] = FREE;
    }

    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        pthread_rwlock_init(&b_lockers[i], NULL);
        free_blocks[i] = FREE;
    }

    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_init(&of_lockers[i], NULL);
...
int state_destroy(void) {
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        pthread_rwlock_destroy(&i_lockers[i]);
    }

    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_destroy(&of_lockers[i]);
    }
...
int inode_create(inode_type i_type) {
    ...
    case T_DIRECTORY: {
        ...
        inode_table[inumber].i_num_hardlinks = 1;
        inode_table[inumber].i_opened = 0;
...
void inode_delete(int inumber) {
    ...
    inode_table[inumber].i_size = 0;
    inode_table[inumber].i_data_block = -1;
    inode_table[inumber].i_num_hardlinks = 0;
...
int clear_dir_entry(inode_t *inode, char const *sub_name) {
    ...
    pthread_rwlock_wrlock(&b_lockers[inode->i_data_block]);
    ...
    for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
        if (!strcmp(dir_entry[i].d_name, sub_name)) {
            ...
            pthread_rwlock_unlock(&b_lockers[inode->i_data_block]);
            return 0;
        }
    }
    pthread_rwlock_unlock(&b_lockers[inode->i_data_block]);
...
int find_in_dir(inode_t const *inode, char const *sub_name) {
    ...
    // Locates the block containing the entries of the directory
    pthread_rwlock_rdlock(&b_lockers[inode->i_data_block]);
    dir_entry_t *dir_entry = (dir_entry_t *)data_block_get(inode->i_data_block);
...
int data_block_alloc(void) {
    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        ...
        pthread_rwlock_wrlock(&b_lockers[i]);
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_rwlock_unlock(&b_lockers[i]);
            return (int)i;
        }
        pthread_rwlock_unlock(&b_lockers[i]);
    }
    ...
    switch (mode) {
        case 'i':
            for (int i = 0; i < INODE_TABLE_SIZE; i++) {
                pthread_rwlock_rdlock(&i_lockers[i]);
                printf("%d | %d, %d, %ld | %s\n", i, inode_table[i].i_node_type, 
                    inode_table[i].i_data_block, inode_table[i].i_size,
                    (freeinode_ts[i] == FREE)? "FREE" : "TAKEN");
                pthread_rwlock_unlock(&i_lockers[i]);
            }
            break;
        case 'o':
            for (int i = 0; i < MAX_OPEN_FILES; i++) {
                printf("%d | %d, %ld | %s\n", i, open_file_table[i].of_inumber,
                    open_file_table[i].of_offset,
                    (free_open_file_entries[i] == FREE)? "FREE" : "TAKEN");
            }
            break;
        case 'r':
            pthread_rwlock_rdlock(&b_lockers[0]);
            dir_entry_t *dir_entry = (dir_entry_t *)data_block_get(0);

            for (int i = 0; i < MAX_DIR_ENTRIES; i++)
                printf("%d | i: %d | n: %s\n", i, dir_entry[i].d_inumber, dir_entry[i].d_name);
            pthread_rwlock_unlock(&b_lockers[0]);
            break;
        default:
            break;
    }
